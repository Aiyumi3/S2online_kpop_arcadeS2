...
class KPopGame extends Phaser.Scene {
	...
	this.physics.add.collider(player, fly, upWay, null, this);
        ...
        heartsText = this.add.text(725, 255, '💚: 24', { fontSize: '9pt', fill: '#c6f50c', fontFamily:'Bembo Bold Italic',
        ...
        scoreText = this.add.text(437, 255, 'score: 0', { fontSize: '9pt', fill: '#c6f50c', fontFamily:'Bembo Bold Italic',
            stroke: 'rgba(194,86,222,0.94)', strokeThickness: 3});
	...
	if(sizeCh == size){
	    ...
	    let scoreTxt = this.add.text(611, 339, ` ${score}`, { fontSize: '11pt', fill: '#ffffff', fontFamily:'Bembo Bold Italic'});
	    scoreTxt.setScrollFactor(0); //is fixed to camera
	    scoreTxt.setDepth(1);
	    let heartTxt = this.add.text(597, 377, ` ${hearts}`, { fontSize: '11pt', fill: '#ffffff', fontFamily:'Bembo Bold Italic'});
	    heartTxt.setScrollFactor(0); //is fixed to camera
            heartTxt.setDepth(1);
		
            let continueText = this.add.text(550, 417, 'continue', { fontSize: '7pt', fill: '#ffffff', fontFamily:'Andy Bold'});
            continueText.setScrollFactor(0); //is fixed to camera
	    continueText.setInteractive();
	    continueText.setDepth(1);
            let reloadText = this.add.text(615, 417, 'reload', { fontSize: '7pt', fill: '#ffffff', fontFamily:'Andy Bold'});
